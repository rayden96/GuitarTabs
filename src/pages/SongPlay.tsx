...
export default function SongPlay() {
  ...
  if (!song || !timeline) return <div className="p-8 text-center text-faint">Loading…</div>
  ...
  return (
    <div className="px-4 pb-72 pt-safe">
      <header className="flex items-center gap-2 py-3">
        <Link to="/" className={`${btnGhost} h-10 w-10`} aria-label="Back to library">
          <BackIcon />
        </Link>
        <div className="min-w-0 flex-1">
          <h1 className="font-display truncate text-xl leading-tight font-bold">{song.title}</h1>
      ...
      {timeline.totalBeats === 0 && (
        <p className="mt-12 text-center text-soft">
          This song has no steps yet.{' '}
          <Link to={`/song/${song.id}/edit`} className="text-wood underline">
            ...
            {section.steps.length > 0 && (
              <button
                ...
                className={`flex h-7 w-7 items-center justify-center rounded-md transition-colors ${
                  loopSectionId === section.id ? 'bg-wood text-[#fdf6e6]' : 'text-faint hover:bg-cream hover:text-soft'
                }`}
      ...
      {currentBeat != null && currentBeat < 0 && (
        <div className="pointer-events-none fixed inset-0 z-40 flex items-center justify-center bg-paper/60">
          <span className="font-display text-9xl font-black text-ember drop-shadow-[0_4px_20px_rgba(253,250,241,0.95)]">
            {beatsPerBar + currentBeat + 1}
          </span>
        </div>
      )}

      {/* control panel */}
      <div className="fixed inset-x-0 bottom-0 z-30 border-t border-line bg-surface/95 shadow-[0_-6px_24px_rgba(54,42,30,0.10)] backdrop-blur">
        <div className="mx-auto max-w-3xl px-4 pt-2 pb-safe">
          {showStrum && displayPattern && (
            <div className="border-b border-line/70 py-1.5">
              <StrumDisplay pattern={displayPattern} activeIndex={isPlaying ? strumIndex : null} large />
            </div>
          )}
          ...
          <div className="flex items-center justify-center gap-1.5 py-2">
            {Array.from({ length: beatsPerBar }, (_, i) => (
              <span
                key={i}
                className={`h-2 rounded-full transition-all ${beatDot === i ? 'w-6 bg-ember' : 'w-2 bg-line'}`}
              />
            ))}
          </div>
          ...
          <div className="flex items-center justify-center gap-3 pb-1">
            ...
            <button
              className="btn-wood flex h-15 w-15 items-center justify-center rounded-full text-[#fdf6e6] shadow-lg shadow-wood/35 transition-transform hover:brightness-110 active:scale-95 disabled:opacity-40"
              onClick={() => (isPlaying ? pause() : play())}
              disabled={timeline.totalBeats === 0}
              aria-label={isPlaying ? 'Pause' : 'Play'}
            >
              {isPlaying ? <PauseIcon width={26} height={26} /> : <PlayIcon width={26} height={26} className="ml-0.5" />}
            </button>
            <button
              className={`flex h-11 w-11 items-center justify-center rounded-lg transition-colors ${
                metroOn ? 'bg-cream text-wood-deep ring-1 ring-line' : 'text-faint hover:bg-cream/60'
              }`}
            ...
            <button
              className={`flex h-11 w-11 items-center justify-center rounded-lg text-xs font-bold transition-colors ${
                countIn ? 'bg-cream text-wood-deep ring-1 ring-line' : 'text-faint hover:bg-cream/60'
              }`}
            ...
            <button
              className={`flex h-11 w-11 items-center justify-center rounded-lg text-sm font-bold transition-colors ${
                showStrum ? 'bg-cream text-wood-deep ring-1 ring-line' : 'text-faint hover:bg-cream/60'
              }`}
          ...
          <div className="flex items-center gap-3 pb-2">
            ...
            <input
              ...
              className="accent-wood flex-1"
              aria-label="Playback tempo"
            />
            <button
              className={`w-12 text-xs font-medium ${tempo !== song.tempo ? 'text-ember hover:text-wood-deep' : 'text-transparent'}`}
